    ...
    while True:
        ...
        if detector.is_recording:
            if detector.out is None:
                ...
                detector.recording_start_time = time.time()
                print(f"Recording started: {filename}")

            detector.out.write(processed_frame)

            # Display recording indicator
            cv2.putText(processed_frame, "RECORDING", (width - 150, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            # Draw a red circle as a recording indicator
            cv2.circle(processed_frame, (width - 20, 20), 10, (0, 0, 255), -1)
        else:
